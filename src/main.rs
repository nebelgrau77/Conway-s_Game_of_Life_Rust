...
fn main() -> ! {
    if let (Some(dp), Some(cp)) = (
        ...
        let mut disp1: GraphicsMode<_> = SSD1306Builder::new().connect_i2c(i2c1).into();
        
        disp1.init().unwrap();

        let mut disp2: TerminalMode<_> = SSD1306Builder::new().size(DisplaySize::Display128x32).connect_i2c(i2c2).into();
        ...
        loop {
            ...
            disp2.write_str(text_buf.as_str()).unwrap();
            ...
            for x in 0..WX {
                for y in 0..HY {
                    ...
                    disp1.set_pixel(x as u32,y as u32,pixel.value);
                }
            }

            

            // Text::new(text_buf.as_str(), Point::new(0, 0)).into_styled(text_style).draw(&mut disp2);
            ...
            while gen < 1001 {
                ...
                for m in 0..96 {
                    for n in 0..16 {
                        disp2.set_pixel(m, n, 0);
                    }
                }
                */
                text_buf = ArrayString::<[u8; 64]>::new();
...
fn matrix_evo(buffer: [u8; 1024]) -> [u8; 1024] {
...
fn pixelgetter(x: i16, y: i16, buffer: [u8; 1024]) -> Pixel {
...
fn counter(buf: &mut ArrayString<[u8; 64]>, gen: u16) {   
    ...
    fmt::write(buf, format_args!("Gen:{}{}{}{}                                                        ", 
    thousands as u8, hundreds as u8, tens as u8, singles as u8)).unwrap();
