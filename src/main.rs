...
//! 
//! //! original MicroPython code ported to STM32F4xx board (this code is for STM32F411CEU6 "black pill")
//! 
...
use crate::hal::{
    ...
    stm32,
    delay::Delay,
    adc::{Adc, config::{AdcConfig, SampleTime, Clock}, Temperature, Vref},
    };
...
fn main() -> ! {
    if let (Some(dp), Some(cp)) = (
        ...
        let adcconfig = AdcConfig::default().clock(Clock::Pclk2_div_8);
        let mut adc = Adc::adc1(dp.ADC1, true, adcconfig);
        adc.enable_temperature_and_vref();

        let tempsensor = Temperature;
        let voltsensor = Vref;
        ...
        let mut rng = SmallRng::seed_from_u64((temp * vref) as u64);
